...
def torch_to_onnx(opt):
    # 加载torch模型
    torch_model = torch.load(opt.model).to(opt.device)
    # 初始化模型
    # torch_model = conv2d().to(opt.device)
    # 初始化输入
...
def onnx_runtime(opt):
    '''
    不适合推理SNN网络，多个时间步下神经元没有记忆功能
    '''
    # 加载输入数据
    data_np = numpy.load(opt.input_data)
    original_input = torch.from_numpy(data_np).to(opt.device)

    # 加载 ONNX 模型
    t1 = time.time()
    onnx_model = onnx.load(opt.model)  # 加载ONNX模型
    onnx.checker.check_model(onnx_model)  # 检查模型的有效性
    t2 = time.time()
    print("初始化时间: {:.2f}ms".format((t2 - t1) * 1000))

    # 使用 ONNX Runtime 进行推理
    ort_session = onnxruntime.InferenceSession(opt.model, providers=["CPUExecutionProvider"])
    ...
    with torch.no_grad():  # 不会记录计算图，速度加快，内存减少
        ...
        # 输出结果
        ort_output = torch.from_numpy(ort_outs[0]).to(opt.device)
        print(f"输出shape: {ort_output.shape}")
        print(f"输出: {ort_output}")


def torch_runtime(opt):
    # 加载输入
    data_np = numpy.load(opt.input_data)
    original_input = torch.from_numpy(data_np).to(opt.device)

    t1 = time.time()
    _, ext = os.path.splitext(opt.model)  # 读取文件扩展名
    if ext == '.onnx':
        onnx_model = onnx.load(opt.model)  # 加载ONNX模型
        torch_model = convert(onnx_model).to(opt.device)  # 转换为PyTorch模型
    elif ext == '.pth' or ext == '.pt':
        torch_model = torch.load(opt.model).to(opt.device)  # 加载PyTorch模型
